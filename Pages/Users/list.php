...
$page_title = 'Usuários';
$service = new UsuarioService($factory);
...
if (!$buscaPorIdExato) {
    if ($busca !== '') {
        ...
        $usuarios = $service->buscarPorNomePaginado($busca, $page, PER_PAGE);
    } else {
        $total    = $service->contarTodos();
        $usuarios = $service->listarPaginado($page, PER_PAGE);
    }
...
function paginaUrl($p, $q) {
    ...
    return '/Pages/Users/list.php?' . http_build_query($params);
}
                ...
                <?php foreach ($usuarios as $usuario) { ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="/Service/Users/delete_action.php?id=<?php echo (int) $usuario->getId(); ?>" class="btn btn-danger btn-xs"
